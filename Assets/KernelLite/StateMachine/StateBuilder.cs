...
namespace Kernel.StateMachine
{
    ...
    public interface IStateBuilder<T, TParent> where T : AbstractState, new()
    {
        ...
        /// <returns>A new state builder object for the new child state</returns>
        IStateBuilder<NewStateT, IStateBuilder<T, TParent>> State<NewStateT>(string name, IConcreteStateDeclaration declaration = null) where NewStateT : AbstractState, new();

		/// <summary>
		/// Create a child state with the default handler type.
		/// </summary>
		/// <param name="name"></param>
		/// <param name="declaration">Declaration of the state to add.</param>
		/// <returns>A state builder object for the new child state</returns>
		IStateBuilder<AbstractRootState, IStateBuilder<T, TParent>> State(string name, IConcreteStateDeclaration declaration = null);
        ...
        /// </summary>
        IStateBuilder<T, TParent> Update(Action<T> onUpdate);
        ...
        where T : AbstractState, new()
    {
		...
		/// <param name="declaration">Declaration of the state to add.</param>
		public StateBuilder(TParent parentBuilder, AbstractState parentState, string name, IConcreteStateDeclaration declaration = null)
        {
            this.parentBuilder = parentBuilder;
		...
		/// <param name="declaration">Declaration of the state to add.</param>
		/// <returns>A new state builder object for the new child state</returns>
		public IStateBuilder<NewStateT, IStateBuilder<T, TParent>> State<NewStateT>(string name, IConcreteStateDeclaration declaration = null) 
            where NewStateT : AbstractState, new()
        {
            return new StateBuilder<NewStateT, IStateBuilder<T, TParent>>(this, state, name, declaration);
        }
		...
		/// <param name="declaration">Declaration of the state to add.</param>
		/// <returns>A state builder object for the new child state</returns>
		public IStateBuilder<AbstractRootState, IStateBuilder<T, TParent>> State(string name, IConcreteStateDeclaration concreteStateDeclaration)
        {
            return new StateBuilder<AbstractRootState, IStateBuilder<T, TParent>>(this, state, name, concreteStateDeclaration);
        }
        ...
        /// Set an action to be called when we update the state.
        /// </summary>
        public IStateBuilder<T, TParent> Update(Action<T> onUpdate)
        {
            state.SetUpdateAction(() => onUpdate(state));
